...
// Broker is the single entry point of the server
...
// HandleSubmission takes care of all actions and give the correct processing
...
// authenticate calls the authentication microservice and sends the appropiate response
...
// logItem calls the logger service
...
// logEventViaRabbit publish to the listener service
func (app *Config) logEventViaRabbit(w http.ResponseWriter, entry LogPayload) {
	err := app.pushQueue(entry.Name, entry.Data)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
// sendMail calls the mail service
